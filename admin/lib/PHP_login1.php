<?php
    ...
    if(empty($_POST['email'])){
        ...
        $sql = "SELECT *FROM usuario WHERE emaUsu='$email' OR dniUsu='$email'"  ;
        $consulta= $conector->query($sql);
        $correo_consulta=$consulta->num_rows;
        if($correo_consulta<1){
            $errors[]= "El Correo o el numero de celular no es valido";
        }else{
            $sql = "SELECT *FROM usuario WHERE emaUsu='$email' OR dniUsu='$email' AND pasUsu = '$password'"  ;
            $consulta= $conector->query($sql);
            ...
            if($pass_consulta<1){
                ...
                if($int_rest <0){                                                           
                    ...
                    $errors[]= "La contraseña no es valida le quedan ".$int_rest." intentos";
                }              
