...
//*********pedidos******** */
/*$('#modal-DetallePedido').on('show.bs.modal',function(event){
    var button=$(event.relatedTarget)
    var id=button.data('id')
    var nom=button.data('nom')
    var precio=button.data('precioProd')
    var cant = button.data('cantProd')
    var modal= $(this)
    modal.find('.modal-title').text('Detalles de Pedido de '+nom)
    modal.find('.modal-body #id').val(id)
    modal.find('.modal-body #nom').val(nom)
    modal.find('.modal-body #precio').val(precio)
})*/
